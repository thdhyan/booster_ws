...
- [x] **T6.2.3** Kick task migration (OmniReset reset paths use removed `write_root_state_to_sim`) — done: cfg imports+instantiates ✅; mdp restored to authentic `89b13a9` (244-line — earlier LFS restore had landed a stale 210-line object missing `reset_ball_omnireset`) + `write_root_pose/velocity_to_sim_index` + `ProxyArray.torch`; **obs-fn audit DONE 2026-09-22**: all 5 ball/goal obs+reward fns now use `.data.*.torch` (isinstance/as_tensor dance removed), quat convention verified **XYZW** end-to-end vs EA source (`root_quat_w` annotated `QUAT_XYZW_ELEMENT_NAMES`; `quat_apply_inverse` docstring `(x,y,z,w)`) → no rotation fix needed; **kick smoke PASSED 2026-09-22 13:19 dl**: 16 envs × 2 it, GPU0, tmux `k1_p4_kick_smoke`, `GUARD_RC=0`, 0 NaN, 0 pxr, 1.8–2.2 s/it, `model_0/1.pt` saved, `clip_0000.mp4` non-black (mean 157.9 / std 51.1) — **T6.2.3 CLOSED**
...
- [x] **T6.3.5** **Static deployability test**: assert every `policy`/student obs group references estimator outputs only — no GT ball/goal terms — DONE 2026-09-22: `tests/test_deployability.py` (pure config introspection: iterates all non-Play K1 gym ids → resolves each ObsTerm func → fails on GT blacklist names or `scene["ball"]`/`scene["goal_bar*"]` source patterns; `teacher` group exempt); passes on all 8 tasks (kick's GT ball terms live only in `teacher`, policy = 42-dim blind proprio ✓). Run: `python tests/test_deployability.py`
- [x] **T6.3.6** Video recorder: every **200 iterations → 30 s clip** → `logs/videos/<run>/iter_XXXX.mp4` + `wandb.Video`; 4 video envs/run; `VideoRecorderCfg` or runner callback (PLAN §3.5) — DONE 2026-09-22 via **EA-native headless Kit viewport recording** (`source='visualizer:kit'`, auto-wired by `apply_video_recording` when `--video` is passed and no recorders are pre-declared). Env cfg pre-sets `sim.visualizer_cfgs = [KitVisualizerCfg(headless=True, origin_type="env", origin_env_index=0, eye=(2.2,-2.2,1.3), lookat=(0,0,0.5), window 640×360)]` — frames env_0's robot; pre-setting it also skips the launcher auto-injection. `fps` auto-resolves to 50 (`1/step_dt`), so CLI `--video_length 1500` = 30 s, `--video_interval 4800` = 200 iters × 24 steps. Clips land `<log_dir>/videos/train/clip_XXXX.mp4`, rsl_rl rglobs them to `wandb.Video` automatically. **PLAN §3.5 deviation (superseded by IL 3.0):** the original "4 dedicated low-res video envs" sensor-camera design is infeasible — IL 3.0's `Camera` forces one render product per env (256 envs → ~1–1.5 GB VRAM → CUDA OOM at first PPO update on the 8 GB GPU); the viewport path renders one frame on demand at capture time instead (zero camera products), G2c smoke verified: rc=0, pxr=0, 40-frame clip non-black with robot framed center
- [x] **T6.3.7** `scripts/train_guard.sh` watchdog (GPU > 7 GB / RAM < 1 GiB / disk < 5 GB → kill) + `systemd-run --scope MemoryMax=9G` wrapper (PLAN §3.7) — fixed + exercised by G1 smoke and all G2 smoke attempts (ManagedOOMPreference fix landed in `16a6803`); 2026-09-22 hardened: limit must be breached **3 consecutive 5 s samples (15 s sustained, `GUARD_HITS` env)** before kill — a single-sample dip to 1015 MB during first-run RTX shader compilation killed an otherwise-healthy G2 attempt (`GUARD_RC=99`); RAM metric = `MemAvailable`
- [x] **T6.3.8** wandb project `booster_k1_soccer_hrl` created; run naming `p{1..5}_*` verified in smoke — DONE 2026-09-22: G2 smoke synced as `2026-09-22_08-03-01_p1_basic_teacher` → `https://wandb.ai/thakk100-dhyan-home/booster_k1_soccer_hrl/runs/9mo4u85n`; log_dir `<ts>_p1_basic_teacher` = wandb run name ✓ (agent cfg `experiment_name=run_name=p1_basic_teacher`)

### T6.4 — Base policies (sequential; each: smoke → train → gate)
- [x] **T6.4.1** **P1 BASIC teacher** `Isaac-Basic-Teacher-K1-v0`: rough terrain + height scan + foot contacts + `random_body_push`; rewards LOCKED table (9 terms); 256 envs / 2000 it → `p1_basic_teacher` — **G2 smoke PASSED 2026-09-22** (16 envs × 3 it rc=0: obs policy 42 / teacher 233 ✓, 9 locked rewards all logged to wandb ✓, push term registered w/o error ✓, videos ✓, 0 pxr ✓); **G3 Newton smokes PASSED 2026-09-22** (16×3 rc=0, windowed + headless: `newton_mjwarp` + newton backends for Articulation/ContactSensor/RayCaster, clips non-black + motion, headless = 0 window lines); **backend decision (user): PhysX for ALL local runs** — Newton measured 2.5–3× slower @256 (4.8–5.0 vs 1.6–2.1 s/iter) + PhysX→Newton ckpt transfer NaN-explodes (fresh-policy probe passed, 0 NaN); full run **finishing headless from `model_1500.pt`** in tmux `k1_p1_teacher_physx_finish` (launched 09:57 CDT; first attempt 09:49 was killed by an OpenCode server restart → launcher now homes the tmux server in its own systemd scope, restart-proof) → 2000 **DONE 2026-09-22**: finish run GUARD_RC=0 (2000 it, model_3500) + auto-chained ext GUARD_RC=0 (3000 it, 512 envs) → **model_6498.pt**; peak −0.27, final ≈ −17, clips non-black — teacher ckpt for T6.4.2 = `model_6498.pt`
- [ ] **T6.4.2** **P1 BASIC student** (blind, 42×K10=420 MLP 512-256-128) distill → `p1_basic_student` — launcher = **Path B** `train_student.py --teacher_checkpoint` (EA `isaaclab train` distill strict-loads the PPO ckpt → `distribution.std_param` crash; fixes `fc3c023`+`108bfd4`: strict=False + output_std guard + video wiring + EA `--headless/--enable_cameras` flag compat); **S1 smoke PASSED 2026-09-23 on spark04 isaac-lab image** (contact-sensor root cause fixed: image `activate_contact_sensors` stops at first rigid body vs nested 6.0 importer → `_spawn_k1_urdf` wrapper applies API to all 23 bodies) → **RUNNING full** `tmux k1_spark_p1` spark04 (dl queue cancelled, GPUs busy); outputs → `logs/spark04/` via `k1_spark_sync` → gdrive; **Gate G3**
- [x] **T6.4.3** **P2 MOVE teacher** `Isaac-Move-Teacher-K1-v0`: rough curriculum + pushes + vel cmds; rewards LOCKED (11 terms); 512 envs / 3000 it → `p2_move_teacher` — **DONE 2026-09-22 14:27 dl**: GUARD_RC=0, 3000 it → **`model_2999.pt`**, reward −2.94 → peak **16.22** → final 14.0, logs+ckpt synced to gdrive; wandb run renamed → `p2_move_teacher` (2026-09-23, API)
- [ ] **T6.4.4** **P2 MOVE student** (48×K10=480) distill → `p2_move_student`; **S2 smoke PASSED 2026-09-23 on spark04 isaac-lab image** (feet_air_time > 0 = Gate-G1 contact criterion on image, ckpt+video+wandb green) → **RUNNING full** `tmux k1_spark_p2` spark04 (dl queue cancelled); **Gate G3**
- [ ] **T6.4.5** **P3 HEAD TRACK** `Isaac-HeadTrack-K1-v0`: deployable 12-dim YOLO obs, 2-dim head actions; rewards LOCKED (6 terms, GT angle = reward-only); DR colors/lighting; 512 envs (64 YOLO) / 2000 it → `p3_head_track`; **Gate G2** (`ball_detect_rate` > 0.9) — authoring NEXT (T6.3.1→3.4 infra first); frozen legs = P1 teacher final ckpt; launch target GPU2 once P2 finishes
