...
- [x] **T6.3.6** Video recorder: every **200 iterations → 30 s clip** → `logs/videos/<run>/iter_XXXX.mp4` + `wandb.Video`; 4 video envs/run; `VideoRecorderCfg` or runner callback (PLAN §3.5) — DONE 2026-09-22 via **EA-native `VideoRecorderCfg` (sensor source)**: P1 scene carries a per-env 320×240 `video_cam` (IL 3.0 Camera requires one prim per env; recorder records `raw[0]` = env_0), `video_recorders` pre-declared on env cfg (`source="sensor:video_cam"`, `video_length=1500` = 30 s @50 Hz, `video_interval=4800` = 200 iters × 24 steps, `fps=50`, prefix `p1`) — pre-declaring **also** keeps the launcher off the pxr-poisoning Kit-visualizer path. G2 smoke wrote `videos/train/p1_0000.mp4` (40 frames, non-black, mean RGB 86.5) + uploaded to wandb (`media/videos/video_1_*.mp4`). Naming deviation: `p1_XXXX.mp4` under `<log_dir>/videos/train/` instead of literal `iter_XXXX.mp4` (EA-native layout, auto-picked-up by rsl_rl's wandb.Video rglob)
