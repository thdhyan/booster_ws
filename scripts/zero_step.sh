...
LOG=/tmp/zero_step_$$.log
timeout 1800 /isaac-sim/python.sh scripts/zero_step_check.py --task "$TASK" --num_envs 4 --steps 3 $EXTRA 2>&1 | tee "$LOG"
grep -q "ZERO_STEP_RESULT=OK" "$LOG" && echo ZERO_STEP_DONE=OK || echo ZERO_STEP_DONE=FAIL
