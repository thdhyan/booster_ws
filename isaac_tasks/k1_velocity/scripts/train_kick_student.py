"""Train the K1 kick STUDENT policy via teacher-student distillation.

Stage 2 of the teacher-student regime (run AFTER the privileged teacher has
been trained):

  1. Train teacher (privileged: ball state + noise-free obs):
       python isaac_tasks/k1_velocity/scripts/train_kick.py \
  ...
  2. Train student (this script — distills teacher into 480-dim proprio
     history):
       python isaac_tasks/k1_velocity/scripts/train_kick_student.py \
           --task Isaac-Kick-Ball-K1-Distill-v0 \
           --teacher_checkpoint logs/rsl_rl/k1_kick_teacher/<run>/model_4999.pt \
           --num_envs 4096 --headless

  3. Export the student for deployment:
       python isaac_tasks/k1_velocity/scripts/play_kick_student.py \
           --task Isaac-Kick-Ball-K1-Distill-v0 \
           --checkpoint logs/rsl_rl/k1_kick_student/<run>/model_*.pt \
           --num_envs 20 --headless --export models/k1_kick_student.pt
...
parser = argparse.ArgumentParser(description="Distill K1 kick student.")
parser.add_argument("--task", type=str, default="Isaac-Kick-Ball-K1-Distill-v0")
...
cli_args_unused = None  # parity with train_kick.py; extend if needed
...
import k1_velocity.tasks.kick  # noqa: F401,E402
...
def main(env_cfg: ManagerBasedRLEnvCfg, agent_cfg):
    ...
    # still serializes — strip them from BOTH model configs (same as train_kick.py).
